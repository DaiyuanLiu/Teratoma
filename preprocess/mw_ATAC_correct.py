...
def correct_barcodes(barcodepath, in_bam, out_bam):
	...
	RT_bc3.close()
	RT_bc4 = open(barcodepath+"/barcode4_96_bc.pickle2","rb")
	...
	with pysam.AlignmentFile(out_bam, "wb", header=bf_head_dict) as outf:
		for r in bf:
			...
			if barcode1 in barcode_list1 and barcode2 in barcode_list2 and barcode3 in barcode_list3 and barcode4 in barcode_list4:
				...
				barcode_corrected = rtbarcode_match1 + rtbarcode_match2 + rtbarcode_match3 + rtbarcode_match4

				r.set_tag('CB', barcode_corrected)
				outf.write(r)
